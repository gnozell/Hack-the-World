...
public class EndMenu : MonoBehaviour 
{
	...
	private void Update()
	{
		...
		if (death) //check if Escape key/Back key is pressed
		{
			paused = !paused;  //unpause the game if already paused
	...
	private void OnGUI()
	{
		...
		if (paused){    
			...
			GUI.Box(new Rect(Screen.width/4, Screen.height/4, Screen.width/2, Screen.height/2), deathReason);
